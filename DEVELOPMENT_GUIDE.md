...
| `GET` | `/logs` | `user_uid` (optional query) | `logs[]` isolated to the user's fingerprint |
| `GET` | `/dashboard-stats` | `user_uid` (optional query) | Weekly watermarking counts grouped by day-of-week |
| `GET` | `/crawl-scan` | `user_uid` (optional query) + `target_url` (optional query) | Live image scrape, pHash matching, and DWT extraction |
...
The watermark timestamp **is unique per upload** — it's generated by `datetime.now(timezone.utc).isoformat()` at the moment `create_payload()` is called (inside `/protect`). If two uploads appear to have the same timestamp, they were uploaded within the same second. This is expected and correct.

To prevent parser crashes in the Dart client (`DateTime.parse()`), the backend standardizes UTC timestamps by replacing the `+00:00` offset returned by `isoformat()` with the standard UTC designator `Z` (yielding e.g. `2026-06-15T19:56:53.806242Z` instead of an invalid suffix mixture like `+00:00Z`).
